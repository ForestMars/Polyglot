...
export const ChatInterface = () => {
            ...
            prev.map((provider) =>
              provider.id === "ollama"
                ? {
                    ...provider,
                    models: modelNames,
                    defaultModel: modelNames[0] || "llama3.2",
                  }
                : provider,
            ),
          );
  ...
  const handleSendMessage = async () => {
    ...
    console.log(
      "🔍 About to call messageRouter.handleMessage() with:",
      input.trim(),
    );
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { ...assistantMessage, isThinking: true, startTime },
      ]);
      ...
      if (FORCE_ENABLE_RAG || enableRAG) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `RAG endpoint returned ${response.status}: ${errorText}`,
            );
          }
          ...
          if (data.results && data.results.length > 0) {
            ...
            if (relevantResults.length > 0) {
              ...
              console.log(
                `🔍 RAG: Retrieved ${relevantResults.length} relevant chunks (filtered from ${data.results.length})`,
              );
              console.log(
                "🔍 First chunk preview:",
                relevantResults[0].content.substring(0, 100) + "...",
              );
              console.log(
              ...
              console.log(
                "🔍 RAG: No relevant results found (all above distance threshold), using general knowledge",
              );
            }
          } else {
  ...
  const handleConversationSelect = useCallback(
    async (conversation: Conversation) => {
      if (
        ...
        conversationState.currentConversation?.id === conversation.id
      ) {
        return;
      }
      ...
      try {
        ...
        const loadedConversation = await conversationState.loadConversation(
          conversation.id,
        );
        ...
        if (
          loadedConversation.currentModel &&
          loadedConversation.currentModel !== selectedModel
        ) {
        ...
        if (
          ...
          loadedConversation.provider !== selectedProvider
        ) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div
        className={`${showSidebar ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-30 w-79 transform bg-white shadow-lg transition-transform duration-300 ease-in-out md:relative md:translate-x-0`}
      >
        <ConversationSidebar
        ...
        >
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <SettingsPanel
              ...
              setSelectedApiKey={async (keyId) =>
                await updateSetting("selectedApiKey", keyId)
              }
              selectedModel={selectedModel}
              setSelectedModel={async (model) =>
                await updateSetting("selectedModel", model)
              }
