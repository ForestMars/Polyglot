...
export class OllamaService {
  ...
  async chat(request: OllamaRequest): Promise<OllamaResponse> {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[OllamaService] chat failed: ${response.status} — ${errorBody}`,
      );
      throw new Error(
  ...
  async generate(
    request: Omit<OllamaRequest, "messages"> & { prompt: string },
  ...
  async healthCheck(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `[OllamaService] Health check response: ${response.status} ${response.statusText} (${endTime - startTime}ms)`,
      );

      if (!response.ok) {
        const errorText = await response.text().catch(() => "No error details");
        console.error(
          `[OllamaService] Health check failed: ${response.status} ${response.statusText} - ${errorText}`,
        );
      }
