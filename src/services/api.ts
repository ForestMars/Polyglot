...
export class ApiService {
  ...
  private getOpenRouterService(apiKey: string): OpenRouterService {
    if (
      ...
      this.openRouterService["apiKey"] !== apiKey
    ) {
  ...
  async sendMessage(request: ChatRequest): Promise<ChatResponse> {
    ...
    try {
      ...
      if (!hasSystem) {
        ...
        const sys = mcpService.getCachedSystemPrompt
          ? mcpService.getCachedSystemPrompt()
          : "";
        if (sys && sys.length > 0) {
    ...
    try {
      switch (provider) {
        ...
        case "openai":
          return await this.handleOpenAIRequest(
            model,
            messages,
            request.apiKey,
          );
        case "anthropic":
          return await this.handleAnthropicRequest(
            model,
            messages,
            request.apiKey,
          );
        case "google":
          return await this.handleGoogleRequest(
            model,
            messages,
            request.apiKey,
          );
        case "openrouter":
          return await this.handleOpenRouterRequest(
            model,
            messages,
            request.apiKey,
          );
        case "together":
          return await this.handleTogetherRequest(
            model,
            messages,
            request.apiKey,
          );
        ...
        case "mistral":
          return await this.handleMistralRequest(
            model,
            messages,
            request.apiKey,
          );
  ...
  private async handleOllamaRequest(
    ...
    try {
      ...
      if (!isHealthy) {
        throw new Error(
          "Ollama is not running. Please start Ollama and ensure it's accessible at the configured URL.",
        );
      }
      ...
      console.log(
        "[handleOllamaRequest] Sending request to Ollama:",
      ...
      console.log(
        `[handleOllamaRequest] Received response in ${endTime - startTime}ms`,
      );
      console.log(
        "[handleOllamaRequest] Response:",
        JSON.stringify(response, null, 2),
      );

      // Check for tool calls via mcpService
      const toolResult = await mcpService.detectAndExecuteTool(
  ...
  async checkProviderHealth(
    ...
    switch (provider) {
      case "ollama":
        const ollamaService = baseUrl
          ? new OllamaService(baseUrl)
          : this.ollamaService;
